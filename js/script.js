window.addEventListener('DOMContentLoaded', () => {
    const tabs = document.querySelectorAll('.tabheader__item'), //біз клик ететін зат
        tabsContent = document.querySelectorAll('.tabcontent'),// контент
        tabsParent = document.querySelector('.tabheader__items');// родитель полный табты камтитын
    ...
    class MenuCards {
        constructor(src, alt, title, descr, price, parentSelector, ...classes) {
            ...
            this.parent = document.querySelector(parentSelector);// дом элемент жатады
            this.transfer = 27;
            this.changeToUAH();
        }
        // Method Convertation
    ...
    getResource('http://localhost:3000/menu')
        .then(data => {
            data.forEach(({ img, altimg, title, descr, price }) => {
                new MenuCards(img, altimg, title, descr, price).render();
            })
        })
    // new MenuCards(
    //     "img/tabs/vegy.jpg",
